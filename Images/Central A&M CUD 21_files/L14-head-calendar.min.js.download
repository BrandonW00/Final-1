(function($){
  function addIconsToPage() {
    ...
    if (socsPageType === 'view_event_in_month') inMonth = '&inMonth=1';

    html = 
'<div class="ezEdit">' +
  '<a href="/vcalendar/?FROMPAGE=Edit_Event&id=' + socsEventID + inMonth + '">' +
    '<img src="/vimages/backend/shared/editArticle.gif" title="Edit Event in Place" alt="Edit Event in Place Icon" \/><\/a>' +
  '<a href="/vcalendar/manager.v?FROMPAGE=Event&ACTION=update_event&ID=' + socsEventID + '&ezEdit=1' + inMonth + '">' +
    '<img src="/vimages/backend/shared/editArticleCS.gif" title="Edit Event in Central Services" alt="Edit Event in CS Icon" \/><\/a>' +
  '<a href="/vcalendar/manager.v?FROMPAGE=Event&ACTION=delete_event&ID=' + socsEventID + '&ezDelete=1' + inMonth + '">' +
    '<img src="/vimages/backend/shared/deleteArticle.gif" title="Delete Event" alt="Delete Event Icon" \/><\/a>' +
'<\/div>';

    if (socsPageType === 'view_event_in_month') {
      if ($('body.maag > div.container').length) {
        // L11 template HTML structure
        $('body.maag > div.container').prepend(html);
      } else if ($('body.monthGlance div.twelve').length) {
        // Mobile template HTML structure
        $('body.monthGlance div.twelve').prepend(html);
      } else {
        $('body').prepend(html);
      }
    } else {
      if ($('#sectionName').length) {
        $('#sectionName').after(html);
      } else if ($('.sub-page-content .breadcrumb').length) {
        // L11 template HTML structure
        $('.sub-page-content .breadcrumb').after(html);
      } else {
        // L12, L14 Template
        $('.calendar-top-menu-with-heading').after(html);
      }
    }
  }

  $(function() {
    // If the global JS variable created and set by the Smarty "signedin" insert function doesn't exist or is false, leave.
    // Added for L14:  Leave if $page == "list" (adding support for creating events from "all events" view, but don't want the "Easy Edit" icons to show up)
    if (typeof socsUserAccess === 'undefined' || !socsUserAccess || socsPageType === 'list') return false;
...
;(function($) {
  $(function() {
    ...
    if ($('.socs-color-picker').length) {

      var $colorField = $('.socs-color-picker');

      // Color Picker jQuery plugin - http://www.digitalmagicpro.com/jPicker/

      // Add a link (may need to get the color from it) and the color picker span
      $colorField.after('<a href="#" id="linkColor" style="display:none;">test<\/a> <span id="colorPicker"><\/span>');

      // If the event doesn't have a specified color, get and use the color of the hidden link just added.
      var color = $colorField.val();
      if (!color) {
        var linkColor = $('#linkColor').css('color');
        color = (linkColor.indexOf('rgb') != -1) ? rgb2hex(linkColor) : linkColor;
      }

      // Tell jPicker the path to the images
      $.fn.jPicker.defaults.images.clientPath='/css/backend/jpicker/images/';

      $('#colorPicker').jPicker({
        window: {
          expandable: true,
          position: {
            x: -($colorField.width() + 10),
            y: $colorField.height() + 5
          }
        },
        color: {
      ...
      function(color, context) { //commitCallback
        var all = color.val('all');
        $colorField.val((all && '#' + all.hex || ''));
      });

      // Align the jPicker graphic with the input field.
      $('.jPicker').css('vertical-align', 'top');

      // Update color picker with value in field when it loses focus
      $colorField.bind('blur', function() {
        var x = $(this).val();
        if (x) $.jPicker.List[0].color.active.val('hex', x);
      });
